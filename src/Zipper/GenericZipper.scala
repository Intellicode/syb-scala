...
*/
  ...
  def left[X : Data, A] (z : ZipperC [X,A]) : Option[ZipperC[X,A]] = {
    z match {
      ...
      case ZipperC(_, CtxtCons(LeftUnit(_), _, _)) => None
      case ZipperC(h1, CtxtCons(LeftCons(l,h2),r,c)) =>
//        Some(ZipperC(h2, CtxtCons[H,RO,X,R](l, RightCons(h1,r)(implicitly[Data[X]]), c)(implicitly[Data[X]]))(implicitly[Data[X]]))
        Some(ZipperC(h2, CtxtUnit[X,A]())(implicitly[Data[X]]))
    }
  }
//CtxtCons[hole, root, parent : Data, rights]
